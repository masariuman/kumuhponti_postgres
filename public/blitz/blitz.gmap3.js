...
var BlitzMap = new function(){
    ...
    var warna_dalam;
    var warna_opacity;
    var warna_strokecolor;
    var warna_strokeopacity;
    var warna_strokeweight;
    var overlaytipe;
    /*****************************************
     *
     * Function Init()
    ...
    function GetContent( overlay ){
        var content =
            '<div style="text-align:center; padding-left:18px;"><h3>'+overlay.title+'</h3></div><div style="border-top:1px dotted #999; white-space: pre;display: block;padding: 9.5px;    margin: 0 0 10px 20px;    font-size: 13px;    line-height: 1.428571429;    word-break: break-all;    word-wrap: break-word;    color: #333333;    background-color: #f5f5f5;    border: 1px solid #cccccc;    border-radius: 4px;    font-family: Monaco, Menlo, Consolas, "Courier New", monospace;">'+overlay.content+'</div>'
    ...
    function getEditorContent( overlay ){
        ...
        var content = '<style>'
            ...
            + '<div style="padding-bottom:3px;">Nama Lokasi:&nbsp;&nbsp;<input type="text" id="BlitzMapInfoWindow_title" value="'+overlay.title+'" style="border:2px solid #dddddd;width:150px;padding:3px;" ></div>'
            + '<div style="padding-bottom:3px;">Deskripsi:<br><textarea id="BlitzMapInfoWindow_content" style="border:2px solid #dddddd;width:250px;height:115px;white-space: pre;">'+overlay.content+'</textarea></div>'
            + '</div>'
        ...
        if( overlay.type == 'polygon' || overlay.type == 'circle' || overlay.type == 'rectangle' ){
            if (warna_dalam === undefined) {
                //nothine todo
            }else {
                overlay.fillColor = warna_dalam;
            }

            if (warna_opacity === undefined) {
                //nothine todo
            }else {
                overlay.fillOpacity = warna_opacity;
            } 
        ...
        if( overlay.type != 'marker' ){
            ...
            if (warna_strokecolor === undefined) {
                //nothine todo
            }else {
                overlay.strokeColor = warna_strokecolor;
            }

            if (warna_strokeopacity === undefined) {
                //nothine todo
            }else {
                overlay.strokeOpacity = warna_strokeopacity;
            }

            if (warna_strokeweight === undefined) {
                //nothine todo
            }else {
                overlay.strokeWeight = warna_strokeweight;
            }
        ...
        content += '<input type="hidden" id="BlitzMapInfoWindow_overlaytype" value="'+ overlay.type +'" >';
        return content;
    }
    ...
    this.closeInfoWindow = function(){
            ...
            overlaytipe = document.getElementById('BlitzMapInfoWindow_overlaytype').value;

            if( overlaytipe != 'marker' ){
                warna_strokecolor = new jscolor.color(document.getElementById('BlitzMapInfoWindow_strokecolor'), {});
                warna_strokeopacity = document.getElementById('BlitzMapInfoWindow_strokeopacity').value;
                warna_strokeweight = document.getElementById('BlitzMapInfoWindow_strokeweight').value;

                if( overlaytipe == 'polygon' || overlaytipe == 'circle' || overlaytipe == 'rectangle' ){
                    warna_dalam = new jscolor.color(document.getElementById('BlitzMapInfoWindow_fillcolor'), {});
                    // warna_dalam = document.getElementById('BlitzMapInfoWindow_fillcolor');
                    warna_opacity = document.getElementById('BlitzMapInfoWindow_fillopacity').value;
                }
            }
            else {
                //do nothing
    ...
    this.SearchBox = function(places) {
       var markers = [];
       // mapObj.setCenter( new google.maps.LatLng( -0.061623626048349894, 109.37841386988248 ) );
       // console.log(coor);
          if (places.length == 0) {
            return;
          }
    ...
    this.tengah = function(x, y, besar){
        mapObj.setCenter( new google.maps.LatLng( y, x ) );
        mapObj.setZoom( besar );
    }
