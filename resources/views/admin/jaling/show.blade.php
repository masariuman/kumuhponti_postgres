...
<div class="block full">
	    <!-- Get Started Content -->
	    	<!-- Datatables Block -->
<form enctype="multipart/form-data" name="validd" method="POST" onsubmit="return kosong()" action="{{ route('kumuh.store') }}">
                            ...
                            <div class="row">
                              ...
                              <div class="col-md-6">
                                 <div class="desc"><b>
                                 	<table>
                                 		...
                                 		<tr>
                                 			<td colspan="3">
                                 				<button id="luas" class="btn btn-primary" type="button">
		                                          <i class="fa fa-calculator"></i> Hitung Luas
		                                        </button>
                                 			</td>
                                 		</tr>
                                    <tr>
                              ...
                              <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="hidden" id="mapData" style="width:100%; height:300px" name="data">{!! (empty($data->data)) ? $tengah->tengah : $data->data !!}</textarea>
                                    <textarea class="hidden" id="kmlString" style="width:100%; height:500px"></textarea>
                                </div>
                                <div class="form-group">
                                	<input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                                	<input class="hidden" type="text" name="luas" id="luasinput" value="{{ $data->luas }}" />
                                	<textarea class="form-control" name="catatan" placeholder="Tulis Catatan Pembaharuan Disini..." style="min-height: 122px;"></textarea>
                                </div>
  ...
  //       $('#pac-input').keydown(function(e){         
   ...
   //         function isEmpty(obj) {
        ...
        function kosong(){
        	$('#generate_text').trigger('click');
        	var valid = document.getElementById("mapData").value;
        	var luas  = document.getElementById("luasinput").value;
        	...
        	if(isEmpty(validation.overlays)) {
				...
				alert("Silahkan dapatkan nilai luas dengan menekan tobol Hitung Luas");
				return false;
			}
          ...
          function setMapData( jsonString ){
            ...
            for( var m = inputData.overlays.length-1; m >= 0; m-- ){
                    if( inputData.overlays[m].type == "polygon" ){
                      ...
                      var tastaluas = tasta[0];
                      // alert(JSON.stringify(tastaluas));
                      // alert(tastaluas.lat);
                      // var tastaluass = ;
                      ...
                      for (var j = 0; j < inputData.overlays[m].paths.length; j++) {
                        ...
                        for (var k = 0; k < inputData.overlays[m].paths[j].length; k++) {
                          var point = inputData.overlays[m].paths[j][k];
                          var nen = new google.maps.LatLng(point.lat,point.lng);
                          ring.push(nen);
                        }
                        rings.push(ring);
                      }
                      // var tastaluass = new Array;

                      Area = google.maps.geometry.spherical.computeArea( ring );
                      document.getElementById('luass').innerHTML = (Area/(10000)).toFixed(3) +" Hektar";
                      document.getElementById('luasinput').value = (Area/(10000)).toFixed(3) +" Hektar";
